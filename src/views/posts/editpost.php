...
<main class="py-15 max-w-4xl mx-auto px-4 flex-1 w-full">
    <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm dark:bg-gray-900 dark:border-gray-700">
        ...
        <?php if (!empty($errors['general'])): ?>
            <div class="mb-6 flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 px-9 py-3 text-xl text-red-400">
        ...
        <form action="/posts/edit/<?= $post->id ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="file" name="image"
                    class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:file:bg-indigo-900/30 dark:file:text-indigo-400 dark:hover:file:bg-indigo-900/50 transition-all cursor-pointer">
                <?php if (!empty($errors['file'])): ?>
                    <p class="mt-1 text-sm text-red-400"><?= htmlspecialchars($errors['file']) ?></p>
                <?php endif; ?>
            </div>

            <div class="flex gap-4">
                <a href="/posts/<?= $post->id ?>" class="w-1/2 text-center bg-gray-50 text-gray-500 font-bold py-4 rounded-xl hover:bg-gray-100 transition-all dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700">
                    Cancel
                </a>
                <button type="submit" class="w-1/2 bg-indigo-600 text-white font-bold py-4 rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:shadow-indigo-900/30">
                    Update Post
                </button>
            </div>
        </form>
